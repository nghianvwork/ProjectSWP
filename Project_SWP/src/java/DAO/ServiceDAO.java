...
public class ServiceDAO extends DBContext {
    ...
    public static void addService(Service s) throws SQLException {
        try {
            ...
            Connection con = new DBContext().getConnection();
            String sql = "INSERT INTO BadmintonService (name, price, description, image_url, status, category) VALUES (?, ?, ?, ?, ?, ?)";
    ...
    // Lấy danh sách loại dịch vụ DISTINCT trong DB
    public List<String> getAllCategories() {
        List<String> list = new ArrayList<>();
        String sql = "SELECT DISTINCT category FROM BadmintonService WHERE category IS NOT NULL";
...
// Lấy dịch vụ có filter & phân trang
    ...
    public List<Service> getServices(String keyword, String status, String category, Double minPrice, Double maxPrice, int offset, int limit) {
        List<Service> list = new ArrayList<>();
        ...
        try (Connection con = new DBContext().getConnection(); PreparedStatement ps = con.prepareStatement(sql.toString())) {
            ...
            while (rs.next()) {
                ...
                list.add(new Service(
                        ...
                        rs.getString("image_url"),
                        rs.getString("status"),
                        rs.getString("category") // <-- lấy category
                ));
            }
        } catch (Exception e) {
            e.printStackTrace();
...
// Đếm dịch vụ có filter
    public int countServices(String keyword, String status, String category, Double minPrice, Double maxPrice) {
        int count = 0;
